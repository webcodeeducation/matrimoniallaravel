...
class AdminController extends Controller
{
    ...
    {
        $admin = Admin::find($request->session()->get('loggedAdmin'));
        $old_photo = $admin->propic;
        // Delete a single file
        File::delete('images/admin/propic/'.$old_photo);
        //print_r($admin);
        if($request->hasFile('proPic')) {
        $image = $request->file('proPic');
        $fileName = rand() .rand() . '.' .$image->getClientOriginalExtension();
        //$photoname = rand().'gallerykmb.'.$image->getClientOriginalExtension();
        $destinationPath = public_path('images/admin/propic');
        //$image->move($destinationPath, $photoname);
        //$image->move('images/admin/propic', $fileName);
        //$admin->propic = '/admin/propic/'.$fileName;
        $admin->propic = $fileName;
        if ($admin->save() == 1) {
            $image->move('images/admin/propic', $fileName);
        }
        }else{
        ...
        print_r($file);
        //echo $fileName = $admin->uname.".".$file->getClientOriginalExtension();
        echo $fileName = rand() . '.' .$file->getClientOriginalExtension();
        $admin->propic = '/admin/propic/'.$fileName;
        if ($admin->save() == 1) {
